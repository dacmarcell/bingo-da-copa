    ...
    <div className="min-h-screen bg-background text-foreground font-body p-4 max-w-md mx-auto pb-24">
      ...
      <form onSubmit={createMatch} className="space-y-2 mb-8">
        <div className="grid grid-cols-2 gap-2">
          ...
          <input
            ...
            maxLength={4}
            placeholder="Sigla A"
            value={form.team_a_code}
            onChange={(e) => setForm({ ...form, team_a_code: e.target.value.toUpperCase() })}
            className="bg-input border border-border px-3 py-2 text-sm font-mono uppercase"
          />
          <input
            required
            maxLength={4}
            placeholder="Sigla B"
            value={form.team_b_code}
            onChange={(e) => setForm({ ...form, team_b_code: e.target.value.toUpperCase() })}
            className="bg-input border border-border px-3 py-2 text-sm font-mono uppercase"
          />
        </div>
        <input
      ...
      <div className="space-y-2 mb-8">
        {matches.map((m) => (
          <div
            key={m.id}
            className="bg-card border border-border p-3 flex justify-between items-center"
          >
            <div>
              ...
              <p className="font-mono text-[10px] text-muted-foreground">
                {new Date(m.starts_at).toLocaleString("pt-BR")} · {m.status}
              </p>
            </div>
            <button
      ...
      <Link
        to="/"
        className="block text-center font-mono text-[10px] uppercase text-muted-foreground"
      >
        ← voltar
      </Link>
    </div>
