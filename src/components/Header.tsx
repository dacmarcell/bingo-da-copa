  ...
  return (
    <nav className="flex items-center justify-between mb-6 sticky top-0 bg-background/80 backdrop-blur-md py-3 z-20 -mx-4 px-4 border-b border-border">
      <Link to="/" className="flex items-center gap-3">
        <div className="size-10 bg-accent rounded-sm flex items-center justify-center font-display text-xl text-accent-foreground">
          B
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {user ? (
          ...
          <Link
            to="/auth"
            className="bg-primary text-primary-foreground font-display text-xs uppercase tracking-widest px-3 py-1.5"
          >
            Entrar
          </Link>
        )}
